...
class Selector
{
  public:
    ...
    std::vector<std::vector<dvec3>> selectPoints(const pcl::PointCloud<pcl::PointXYZRGBNormal> &in_cloud, std::vector<std::vector<dvec3>> &color_list, int num_curves)
    {
        std::vector<std::vector<dvec3>> result;
        std::vector<std::vector<dvec3>> sliced_cloud_color;
        std::vector<std::vector<dvec3>> sliced_cloud = sliceCloud(in_cloud, sliced_cloud_color, num_slices);
        // std::cout<<sliced_cloud_color.size()<<std::endl;
        for(int i=0; i<sliced_cloud.size(); i++)
        {
            // std::cout<<sliced_cloud_color[i].size()<<std::endl;
            std::vector<std::vector<dvec3>> lines_color;
            std::vector<std::vector<dvec3>> lines = buildLine(sliced_cloud[i], sliced_cloud_color[i], lines_color, num_max_pts, num_max_line);
            result.insert( result.end(), lines.begin(), lines.end() );
            // std::cout<<lines_color.size()<<std::endl;
            color_list.insert( color_list.end(), lines_color.begin(), lines_color.end() );
        }
    ...
    std::vector<std::vector<dvec3>> sliceCloud(const pcl::PointCloud<pcl::PointXYZRGBNormal> &in_cloud, std::vector<std::vector<dvec3>> &sliced_cloud_color, int num_slices)
    {
        std::vector<std::vector<dvec3>> result (num_slices);
        sliced_cloud_color =  std::vector<std::vector<dvec3>>(num_slices);
        ...
        for(int i=0;i<num_pts;i++)
        {
            ...
            sliced_cloud_color[index].push_back(dvec3(in_cloud.points[i].r, in_cloud.points[i].g, in_cloud.points[i].b));
        }

        return result;
    }

    std::vector<std::vector<dvec3>> buildLine(const std::vector<dvec3> &in_pts, const std::vector<dvec3> &in_colors, std::vector<std::vector<dvec3>> &out_colors, int num_max_pts, int num_max_line)
    {
        std::vector<std::vector<dvec3>> result;
        ...
        for(int i=0; i<num_max_line; i++)
        {
            ...
            for(int j=0; j<num_max_pts; j++)
            {
                ...
                c_line_color.push_back(in_colors[start_id]);
            ...
            if(c_line.size() >= 4)
            {
                ...
                out_colors.push_back(c_line_color);
                // std::cout<<c_line_color.size()<<std::endl;
                is_new_line = true;
            }
